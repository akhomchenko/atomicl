...
- [x] Validate sync, tests, and build with the new workflow.
...
- Validation for that packaging change passed with `uv build`, isolated install from the built sdist, `uv sync`, and `uv run pytest tests/test_atomicl.py`.
